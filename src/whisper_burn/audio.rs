...
fn get_mel_filters_device<B: Backend>(
    ...
    let ramps =
        mel_f.clone().unsqueeze::<2>().transpose().repeat(1, n_ffefreqs) - fftfreqs.unsqueeze();
    ...
    let lower = -ramps.clone().slice([0..n_mels])
        / fdiff.clone().slice([0..n_mels]).unsqueeze::<2>().transpose();
...
pub fn stfft<B: Backend>(
    ...
    let right_pad = reverse(
        input.clone().slice([0..n_batch, (orig_input_size - pad - 1)..(orig_input_size - 1)]),
        1,
    );
